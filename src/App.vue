      ...
      <main class="flex flex-col h-screen overflow-hidden relative">
        <header class="h-16 bg-white dark:bg-slate-800 border-b border-slate-200 dark:border-slate-700 flex items-center justify-between px-8 shrink-0 z-20">
          ...
          <div class="ml-4 flex items-center gap-4">
            <button @click="showTaskStatusNotification" class="text-slate-400 hover:text-indigo-600 transition-colors cursor-pointer relative">
              <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6">
                <path stroke-linecap="round" stroke-linejoin="round" d="M14.857 17.082a23.848 23.848 0 0 0 5.454-1.31A8.967 8.967 0 0 1 18 9.75V9A6 6 0 0 0 6 9v.75a8.967 8.967 0 0 1-2.312 6.022c1.733.64 3.56 1.085 5.455 1.31m5.714 0a24.255 24.255 0 0 1-5.714 0m5.714 0a3 3 0 1 1-5.714 0" />
              </svg>
              <span v-if="taskStore.tasks.length > 0" class="absolute top-0 right-0 block h-2 w-2 rounded-full bg-rose-500 ring-2 ring-white dark:ring-slate-800"></span>
            </button>
...
import { useTaskStore } from './stores/taskStore'; // NEW: Imported the task store
...
const taskStore = useTaskStore(); // NEW: Initialized task store

// Initialize global system features
...
const showTaskStatusNotification = () => {
  ...
  if (totalTasks === 0) {
    triggerNotification('All Caught Up! 🌟', { 
      body: 'You have no active tasks in this project right now.',
      icon: 'success'
    });
  } else if (highPriority > 0) {
    triggerNotification('Action Required ⚠️', { 
      body: `You have ${highPriority} High Priority task(s) waiting for you!`,
      icon: 'warning'
    });
  } else {
    triggerNotification('Daily Update 📊', { 
      body: `You are currently tracking ${totalTasks} task(s). Keep up the good work!`,
      icon: 'info'
    });
  }
};
