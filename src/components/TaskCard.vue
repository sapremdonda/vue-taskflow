<template>
  <div @click="$emit('view', task)" class="bg-white dark:bg-slate-800 p-4 rounded-lg shadow-sm border border-slate-200 dark:border-slate-700 cursor-grab active:cursor-grabbing hover:shadow-md transition-shadow group">
    ...
    <div class="flex justify-between items-start mb-2">
      ...
      <div class="flex gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
        <button @click.stop="$emit('edit', task)" class="cursor-pointer text-slate-400 hover:text-indigo-600 dark:hover:text-indigo-400 transition-colors p-1">
        ...
        <button @click.stop="$emit('delete', task.id)" class="cursor-pointer text-slate-400 hover:text-rose-600 dark:hover:text-rose-400 transition-colors p-1">
